...
const emptyForm = {
    modelId: '', modelName: '', description: '', modelLink: '', modelType: '', propertyKey: '', maxTokens: '', retirementDate: '',
...
const sidebarSections = [
    { label: 'OVERVIEW', items: [{ name: 'Home', icon: I.home, href: '/persona/admin' }] },
    {
        label: 'PLATFORM MANAGEMENT', items: [
            ...
            { name: 'Model Onboarding', icon: I.box, active: true, href: '/persona/admin/models' },
...
export default function ModelOnboarding({ theme, onToggleTheme }) {
    ...
    const filtered = models.filter(m => {
        ...
        if (searchQuery) {
            ...
            return m.name.toLowerCase().includes(q) || m.vendor.toLowerCase().includes(q) || m.type.toLowerCase().includes(q)
        }
        return true
    })
        ...
        <div className="dash">
            ...
            <nav className={`dash__sidebar ${sidebarOpen ? 'dash__sidebar--open' : ''}`}
                ...
                <div className="dash__sidebar-top">
                    ...
                    <button className={`dash__sidebar-toggle ${sidebarOpen ? 'dash__sidebar-toggle--open' : ''}`} onClick={() => { setSidebarOpen(o => !o); setSidebarPeek(false) }} aria-label={sidebarOpen ? 'Collapse sidebar' : 'Expand sidebar'}>
            ...
            <div className="dash__content">
                ...
                <header className="dash__header">
                    ...
                    <div className="dash__header-right">
                        ...
                        <button className="dash__icon-btn" aria-label="Settings">{I.settings}</button>
                        <button className="dash__icon-btn" aria-label="Notifications">
                            {I.bell}<span className="dash__notif-dot" />
                        </button>
                                ...
                                <div key={m.id} className={`ml__row ${m.status === 'pending' ? 'ml__row--pending' : ''}`} style={{ '--sc': sm.color, '--sc-bg': sm.bg, '--sc-border': sm.border }}>
                                    ...
                                    <div className="ml__col ml__col--name">
                                        ...
                                        <div className="ml__name-group">
                                            ...
                                            <span className="ml__vendor">{m.vendor} · {m.version}</span>
                                        </div>
                                    </div>
